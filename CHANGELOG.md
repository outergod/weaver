...
- **`weaver edit-json <PATH> --from <PATH-or-dash> [--socket <PATH>]`** — JSON-driven equivalent of `weaver edit`. `--from -` reads stdin; `--from <PATH>` reads a named file. Pre-dispatch wire-frame size check rejects oversized batches (>64 KiB serialised) before they reach the codec.
...
- **Diagnostic codes** added in `core/src/cli/errors.rs`: `WEAVER-EDIT-001` (buffer not opened — pre-dispatch inspect-lookup returned `FactNotFound`), `WEAVER-EDIT-002` (invalid `<RANGE>` grammar OR odd-count variadic pairs), `WEAVER-EDIT-003` (malformed edit-json input), `WEAVER-EDIT-004` (serialised BufferEdit exceeds 64 KiB wire-frame limit). All exit 1.
