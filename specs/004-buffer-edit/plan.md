...
**Scale/Scope**: ~700–1100 new LOC across `core` and `buffers`, plus e2e tests. 1 new `EventPayload` variant (`BufferEdit`); 3 new struct types (`TextEdit`, `Range`, `Position`); 2 new CLI subcommands (`edit`, `edit-json`); 1 new pub(crate) handler on the buffers publisher (`dispatch_buffer_edit` + `BufferEditOutcome` + `apply_edits` on `BufferState`); ~5 new e2e tests (single-edit, atomic-batch, sequential-100, stale-drop, buffer-not-opened) + 1 property test (SC-406 emitter equivalence). No new fact families. Bus protocol 0.3.0 → 0.4.0 (MAJOR).
