...
- [ ] T010 [US1] {latency:interactive} Wire reader-loop arm in `buffers/src/publisher.rs::reader_loop` to dispatch `BusMessage::Event(Event { payload: EventPayload::BufferEdit { entity, version, edits }, id: event_id, .. })` through `dispatch_buffer_edit`. Depends on T009. On each outcome:
...
- **US1 (P1)**: Independent after Phase 2. Mostly self-contained; T013 (handler) depends on T011 + T012; T015 + T016 are e2e tests at story end.
