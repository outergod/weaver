...
**Primary Dependencies**: existing — `tokio`, `ciborium`, `serde` + `serde_json`, `clap` derive, `miette`, `thiserror`, `tracing` + `tracing-subscriber`, `proptest`, `vergen`, `crossterm`, `uuid` (v4), `humantime`, `tempfile` (workspace dev-dep declared at root, already consumed by slice-002 crates — inherited via `tempfile.workspace = true` in `buffers/Cargo.toml` under `[dev-dependencies]`); **new** — `sha2` (content digesting — see `research.md` §1).
