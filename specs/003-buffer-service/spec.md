...
4. **Given** the service is running and the file is deleted mid-session, **When** the next observation tick fires, **Then** the service emits a single `Lifecycle(Degraded)` + `buffer/observable=false` pair (edge-triggered, not per-tick), and the TUI renders the buffer with an `[observability lost]` badge.
...
- **File is unreadable *mid-session* (permissions change, filesystem unmount, file deleted)**: service emits `Lifecycle(Degraded)` + `buffer/observable=false` for the affected entity *exactly once* on the healthy→degraded transition (edge-triggered per slice 002 F21); subsequent failed observations remain silent until recovery.
...
- **FR-004**: The service MUST announce its lifecycle (`started`, `ready`, `degraded`, `unavailable`, `stopped`) through the `watcher/status` fact family, keyed by the service's per-invocation instance entity. The vocabulary matches slice 002 exactly.
...
- **FR-007**: The service MUST maintain exactly one bus connection for the lifetime of the process, regardless of how many buffers are open. All authority claims ride on that single connection — identity is conn-bound per slice 002 F14; authority is per-entity per F10.
...
- **FR-016**: When a file becomes unreadable mid-session (permissions change, file deleted, filesystem unmount), the service MUST publish a single `Lifecycle(Degraded)` + `buffer/observable=false` pair on the healthy→degraded transition (edge-triggered, not per-tick). Recovery (file becomes readable again) MUST publish `Lifecycle(Ready)` + `buffer/observable=true` on the first successful observation.
...
- **Degradation taxonomy**: The buffer service publishes lifecycle transitions via the standard lifecycle messages (`started` → `ready` → optional `degraded` ↔ `ready` ↔ `degraded` → `unavailable` → `stopped`; `restarting` reserved). **Degraded** applies when the service remains connected to the bus but cannot observe one or more buffer files authoritatively (transient filesystem error, file temporarily unreadable, mid-rename race). **Unavailable** applies when the service has lost *all* observable buffers or is exiting.
- **Failure facts**:
...
- **Bootstrap publication order has a shared causal parent.** On successful open of each buffer, the service publishes `buffer/path` → `buffer/byte-size` → `buffer/dirty=false` → `buffer/observable=true` → `watcher/status=ready` with a single synthesized poll-tick event id as the shared `causal_parent`, matching slice 002's repository-bootstrap pattern. This lets `why?` walk the bootstrap as one transaction.
