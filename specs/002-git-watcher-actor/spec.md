...
- Q (from /speckit.analyze C1): FR-015 as originally written required the system to "MUST reconnect and re-establish the watcher's fact authority" after a core restart, but the plan's research §5, `contracts/bus-messages.md` failure-modes table, and the task list all commit to "watcher exits on bus EOF; auto-reconnect out of scope." Which is authoritative? → A: The design decision is authoritative. FR-015 is rewritten to describe what the chosen design actually guarantees: no new facts may carry a stale instance identifier from a prior session. Automatic reconnection is explicitly deferred to a future slice; operator-initiated restart is the current recovery path, and a re-launched watcher obtains a fresh UUID v4 instance identifier (per FR-001 / Clarification Q3), which trivially satisfies the stale-identity prohibition.
...
- **FR-015**: When the core restarts with the watcher attached, the watcher MUST NOT produce any new facts carrying a stale instance identifier from a prior session. Automatic reconnection is out of scope for this slice (per Clarification C1); the watcher MAY exit on bus EOF, requiring operator-initiated restart. A re-launched watcher obtains a fresh UUID v4 instance identifier per FR-001 / Clarification Q3, which trivially satisfies the stale-identity prohibition. A future slice introducing auto-reconnect MUST re-announce the watcher's identity explicitly on each reconnect attempt and MUST NOT reuse a prior session's instance identifier.
