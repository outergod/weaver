...
- [X] T064 [US2] Core-authored and TUI-authored fact rendering: for `ActorIdentity::Core` / `ActorIdentity::Tui`, JSON emits `{ "asserting_kind": "core" }` / `{ "asserting_kind": "tui" }` without identifier fields — extended to ALL variants per Phase-4 Option C: `asserting_kind` is always present; identifier fields are populated only for the slice's emitted kinds

### Scenario tests (Phase 4)

- [X] T065 [P] [US2] Scenario test in `core/tests/inspect/behavior_authored.rs`: start core; trigger a `BufferEdited` event; run `weaver inspect 1:buffer/dirty --output=json`; assert output contains `asserting_behavior: "core/dirty-tracking"` and NO `asserting_service` field
- [X] T066 [P] [US2] Scenario test in `tests/e2e/git_watcher_inspect.rs`: start core + watcher; run `weaver inspect <repo-eref>:repo/dirty --output=json`; assert output contains `asserting_service: "git-watcher"` and `asserting_instance` is a valid UUID v4 matching the watcher's startup log
- [X] T067 [P] [US2] Scenario test in `core/tests/inspect/structured_always.rs`: across ALL fact families (`buffer/*`, `repo/*`, `watcher/*`), `weaver inspect` output NEVER renders an opaque or raw-string actor identity. Enforced at the JSON level (no field value matching `^External\(`)
...
- [X] T068 [P] [US2] Property test in `core/tests/property/inspect_identity.rs`: for any `ActorIdentity` value, round-trip through `weaver inspect`'s JSON emitter then re-parse; the actor identity can be unambiguously reconstructed from the emitted fields
...
- [X] T073 Run the `quickstart.md` procedure manually end-to-end (the three-terminal walkthrough); confirm each SC-001..SC-006 criterion passes; take notes on any friction for future quickstart revisions — walked through 2026-04-23: SC-001 cold-start pass; SC-002 mutation→TUI propagation pass across dirty/clean + detached-HEAD transitions; SC-003 structured `asserting_service` / `asserting_instance` (UUID v4) shape verified alongside behavior-authored `asserting_behavior` rendering; SC-005 watcher disconnect retracted `repo/*` facts cleanly (case a — full retract); SC-006 `cargo test --workspace` green (39 test-result lines). No friction observed; no quickstart revisions needed
