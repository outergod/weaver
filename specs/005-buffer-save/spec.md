...
- **FR-018**: All seven `WEAVER-SAVE-NNN` diagnostics MUST emit structured `tracing` records with fields: `entity`, `path`, `event_id` (the core-stamped ID per FR-022), and a code-specific detail field (e.g., `expected_inode` / `actual_inode` for -005, `errno` / `os_error` for -003/-004). Format consistent with slice-004's diagnostic discipline.
