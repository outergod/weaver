...
- **FR-007**: The CLI MUST expose a status command supporting an output-format flag (`--output=<format>`, also accepts `-o` shorthand per `contracts/cli-surfaces.md`); `json` MUST be supported on Day 1; the output structure MUST mirror the bus fact/event vocabulary.
  - *Scope clarification (per L2 P5):* This CLI structured output is a one-shot snapshot interface for shell scripts, build systems, and human-adjacent tooling. It is **not** the primary integration surface for continuous machine consumers (agents, monitoring, introspection clients) — those participate on the bus via dedicated services and inherit the bus's delivery-class guarantees, provenance, retraction, lifecycle signals, and `why?` walk-back. STDOUT cannot represent streaming or causal continuity.
- **FR-008**: The system MUST expose an inspection capability — addressed at the **bus protocol level** as a request/response — that allows any client to query, for any currently-asserted fact, the source event, the asserting behavior's identifier, and a timestamp.
  - *Implementation shape (per L2 P5):* The inspection request is a bus message any subscriber can issue. The TUI uses it directly. The CLI (`weaver inspect <fact-ref>`) is a thin wrapper over the same bus call — it does not duplicate the inspection logic. A future introspection / agent service will use the same request/response shape without the CLI in the loop.
...
- **Future introspection / agent service**: a dedicated service for continuous machine consumers (agents, monitoring tools, LLM integrations) is anticipated in a later slice, per L2 P5's CLI-vs-bus boundary. This slice does not deliver that service, but FR-008's bus-request/response shape is intentionally designed so the future service can subscribe to and issue the same calls without protocol rework. The CLI's `inspect` and `status --output=json` remain valid for one-shot scripting; they are not the agent integration surface.
